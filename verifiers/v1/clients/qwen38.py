...
def create_qwen38_renderer_pool(
    tokenizer_name_or_path: str,
    config: Qwen36RendererConfig | None,
    *,
    size: int,
    chat_template_kwargs: Mapping[str, Any] | None = None,
) -> RendererPool:
    """Build Prime's default-xhigh Qwen3.8 renderer without global registration."""
    config_values: dict[str, Any] = {
        "enable_thinking": True,
        "preserve_thinking": True,
    }
    if config is not None:
        config_values.update(config.model_dump(exclude={"name"}, exclude_unset=True))
